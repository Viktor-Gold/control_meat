 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */rn._jsonSchemaVersion="firestore/querySnapshot/1.0",rn._jsonSchema={type:re("string",rn._jsonSchemaVersion),bundleSource:re("string","QuerySnapshot"),bundleName:re("string"),bundle:re("string")};function Bh(n,e,t){n=en(n,oe);const r=en(n.firestore,gs),s=$E(n.converter,e),o=LE(r);return HE(r,[ME(o,"setDoc",n._key,s,n.converter!==null,t).toMutation(n._key,We.none())])}function jh(n,...e){n=we(n);let t={includeMetadataChanges:!1,source:"default"},r=0;typeof e[r]!="object"||hu(e[r])||(t=e[r++]);const s={includeMetadataChanges:t.includeMetadataChanges,source:t.source};if(hu(e[r])){const d=e[r];e[r]=d.next?.bind(d),e[r+1]=d.error?.bind(d),e[r+2]=d.complete?.bind(d)}let o,a,u;if(n instanceof oe)a=en(n.firestore,gs),u=uo(n._key.path),o={next:d=>{e[r]&&e[r](WE(a,n,d))},error:e[r+1],complete:e[r+2]};else{const d=en(n,Ms);a=en(d.firestore,gs),u=d._query;const p=new Fh(a);o={next:T=>{e[r]&&e[r](new rn(a,p,d,T))},error:e[r+1],complete:e[r+2]},qE(n._query)}const h=kh(a);return AE(h,u,s,o)}function HE(n,e){const t=kh(n);return RE(t,e)}function WE(n,e,t){const r=t.docs.get(e._key),s=new Fh(n);return new Dt(n,s,e._key,r,new zn(t.hasPendingWrites,t.fromCache),e.converter)}(function(e,t=!0){vg(mn),sn(new Ot("firestore",((r,{instanceIdentifier:s,options:o})=>{const a=r.getProvider("app").getImmediate(),u=new gs(new Rg(r.getProvider("auth-internal")),new Cg(a,r.getProvider("app-check-internal")),Hg(a,s),a);return o={useFetchStreams:t,...o},u._setSettings(o),u}),"PUBLIC").setMultipleInstances(!0)),dt(uu,lu,e),dt(uu,lu,"esm2020")})();const GE={apiKey:"AIzaSyDxQ_wfnt43fOx_Xp2uWWPftpySs2JRzVo",authDomain:"control-meat.firebaseapp.com",projectId:"control-meat",storageBucket:"control-meat.firebasestorage.app",messagingSenderId:"604670453387",appId:"1:604670453387:web:921e97a1a89c7a2d82568e"},qh=Su(GE),ko=Tg(qh),Vo=kE(qh);//! МОДАЛЬНОЕ ОКНО АВТОРИЗАЦИИ
const fn=document.querySelector("#modal_window"),KE=document.querySelector("#loginInput"),du=document.querySelector("#passwordInput"),QE=document.querySelector("#loginBtn"),fu=document.querySelector("#errorMsg");//! Блок интерфейса, пока не введены правильные данные
document.body.style.overflow="hidden";//! Проверка логина и пароля
async function $h(){const n=KE.value.trim(),e=du.value.trim();try{await am(ko,n,e),fn.style.display="none",document.body.style.overflow="auto",fu.textContent=""}catch{fu.textContent="Неверный логин или пароль",du.value=""}}//! При клике на кнопку "Войти"
QE.addEventListener("click",$h);//! При нажатии Enter
fn.addEventListener("keydown",n=>{n.key==="Enter"&&$h()});//! Кнопки
const JE=document.querySelector("#addBtn"),YE=document.querySelector("#saleBtn");//! Вес для добавления и продаж
const pu=document.querySelector("#addValue"),mu=document.querySelector("#saleValue");//! Таблица остатков
const Do=document.querySelector("#balance"),XE=document.querySelector("#summary");//! Ревизия
const ZE=document.querySelector("#revisionBtn"),eT=document.querySelector("#revisionValue");let Kr="",Qr="",zh=0,Jr="";//! Открыть/Закрыть кастомный селектор
function No(n){const e=document.querySelector(n),t=e.querySelector(".custom_select"),r=e.querySelector(".select_option");t.addEventListener("click",()=>{r.style.display=r.style.display=="block"?"none":"block"}),document.addEventListener("click",s=>{e.contains(s.target)||(r.style.display="none")})}No("#addMeat");No("#saleMeat");No("#revisionMeat");//! Настройка селекторов выбора позиции в разных блоках
function Oo(n,e){const t=document.querySelector(n),r=t.querySelector(".custom_select .selected");t.querySelectorAll(".select_option li").forEach(o=>{o.addEventListener("click",()=>{r.textContent=o.textContent;const a=String(o.dataset.value),u=Number(o.dataset.loss);e==="add"?(Qr=a,zh=u):e==="sale"?Jr=a:e==="revision"&&(Kr=a);const h=o.closest(".select_option");h.style.display="none"})})}Oo("#addMeat","add");Oo("#saleMeat","sale");Oo("#revisionMeat","revision");function xs(n){const e=Do.querySelector(`div[data-value="${n}"]`);if(!e)return null;const t=e.nextElementSibling,r=t?.nextElementSibling,s=r?.nextElementSibling;return{nameDiv:e,weightDiv:t,priceDiv:r,sumDiv:s}}function tT(n,e){const t=n*e/100;return n-t}function gr(){let n=0;Do.querySelectorAll("div[data-value]").forEach(t=>{const r=t.nextElementSibling,s=r.nextElementSibling,o=s.nextElementSibling,a=Number(r.textContent),u=Number(s.textContent),h=a*u;o.textContent=h.toFixed(2),n+=h}),XE.textContent=`Итоговая сумма: ${n.toFixed(2)} ₽`}ZE.addEventListener("click",()=>{const n=Number(eT.value);if(!Kr)return alert("Выберите позицию для ревизии.");if(isNaN(n)||n<0)return alert("Введите корректный остаток.");const e=xs(Kr);if(!e||!e.weightDiv)return alert("Позиция не найдена.");e.weightDiv.textContent=n.toFixed(2);const t=Number(e.priceDiv.textContent);Lo(Kr,n,t),gr()});JE.addEventListener("click",()=>{const n=Number(pu.value);if(!Qr)return alert("Выберите позицию для добавления.");if(!n||n<=0)return alert("Введите корректный вес.");const e=tT(n,zh),t=xs(Qr);if(!t||!t.weightDiv)return alert("Позиция не найдена.");const s=Number(t.weightDiv.textContent)+e;t.weightDiv.textContent=s.toFixed(2),pu.value="";const o=Number(t.priceDiv.textContent);Lo(Qr,s,o),gr()});YE.addEventListener("click",()=>{const n=Number(mu.value);if(!Jr)return alert("Выберите позицию для продажи.");if(!n||n<=0)return alert("Введите корректный вес продажи.");const e=xs(Jr);if(!e||!e.weightDiv)return alert("Позиция не найдена.");const t=Number(e.weightDiv.textContent);if(n>t)return alert("Недостаточно остатка!");const r=t-n;e.weightDiv.textContent=r.toFixed(2),mu.value="";const s=Number(e.priceDiv.textContent);Lo(Jr,r,s),gr()});gr();Zu(ko,n=>{n?(fn.style.display="none",document.body.style.overflow="auto"):(fn.style.display="flex",document.body.style.overflow="hidden")});Zu(ko,async n=>{n?(fn.style.display="none",document.body.style.overflow="auto",await nT()):(fn.style.display="flex",document.body.style.overflow="hidden")});const Hh=bE(Vo,"balance");jh(Hh,n=>{n.forEach(e=>{const t=e.data(),r=xs(e.id);!r||!r.weightDiv||!r.priceDiv||(r.weightDiv.textContent=String(t.weight),r.priceDiv.textContent=String(t.price))}),gr()});async function Lo(n,e,t){await Bh(bh(Vo,"balance",n),{weight:e,price:t})}async function nT(){if(!(await new Promise(t=>{jh(Hh,r=>t(r))})).empty)return;console.log("Инициализация Firestore начальными данными");const e=Do.querySelectorAll("div[data-value]");for(const t of e){const r=t.dataset.value;if(!r)continue;const s=t.nextElementSibling,o=s.nextElementSibling,a=Number(s.textContent)||0,u=Number(o.textContent)||0;await Bh(bh(Vo,"balance",r),{weight:a,price:u})}}
